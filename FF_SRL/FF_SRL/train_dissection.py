"""
Train SAC agent for tumor dissection task (push-based).

Usage:
    python -m FF_SRL.train_dissection
    python -m FF_SRL.train_dissection --total_timesteps 200000 --max_steps 200
"""
...
from FF_SRL.env_tumor_dissection import TumorDissectionEnv


def make_env(args):
    """Create and wrap the environment."""
    env = TumorDissectionEnv(
        device=args.device,
...
def main():
    parser = argparse.ArgumentParser(description="Train SAC for tumor dissection")
    ...
    parser.add_argument("--action_strength", type=float, default=0.04,
                        help="Dissector displacement per step (cm)")
    ...
    # Output
    parser.add_argument("--save_dir", type=str, default="./output/dissection_sac")
    ...
    print("=" * 60)
    print("TUMOR DISSECTION SAC TRAINING (Push-based)")
    print("=" * 60)
    print(f"Save dir: {save_dir}")
    print(f"Total timesteps: {args.total_timesteps}")
    print(f"Action strength: {args.action_strength} cm/step")
    print(f"Push radius: {args.push_radius} cm")
    print(f"Push strength: {args.push_strength} cm")
